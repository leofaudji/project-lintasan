...
function CreateFileExport($cExtention = '.csv', $export_name=""){
  $cDir = CI::$APP->config->item('bcore_tmp') ;
...
function __construct($paper='LETTER',$orientation='portrait',$export=0,$vaopt=array(),$lpagenumber=FALSE,$nPaperIncWidth=0,$nPaperIncHeight=0){
  ...
  if(!empty($vaopt)){
    if(isset($vaopt['mtop']))  $nTopMargin       = $vaopt['mtop']/10 ;
    if(isset($vaopt['mleft'])) $nLeftMargin      = $vaopt['mleft']/10 ;
    if(isset($vaopt['mbottom'])) $nBottomMargin  = $vaopt['mbottom']/10 ;
    if(isset($vaopt['mright'])) $nRightMargin    = $vaopt['mright']/10 ;
  }
  if(!isset($vaopt['export_name'])) $vaopt['export_name'] = "" ;

  if($this->nExport == "1"){
    $this->CreateFileExport(".csv", $vaopt['export_name']) ;
  }else if($this->nExport == "2"){
    $this->objPHPExcel = new PHPExcel();
    $this->CreateFileExport(".xlsx", $vaopt['export_name']) ;
  }

  if(strtolower($orientation) == "p") $orientation = "portrait" ;
  if(strtolower($orientation) == "l") $orientation = "landscape" ;
  ...
  //Courier-Oblique, Times-Roman, Courier
...
function ezSetMargins($top,$bottom,$left,$right){
  ...
  $this->ez['rightMargin']=$right;
   // echo("Y : ".$this->y . "| m :" . ($this->ez['pageHeight']-$top));
...
function ezStream($options=''){
  if($this->nExport == 0){
    ...
    echo('<a href="' . base_url($this->cfileexport) . '" style="text-align:center">Click Here</a> to download file CSV') ;
  } else if($this->nExport == 2){
    //AUTO SIZE
    $col = 'A';
    while(true){
    ...
    echo('<a href="' . base_url($this->cfileexport) . '" style="text-align:center">Click Here</a> to download file XLSX') ;
  }
}
...
function ezLogoHeaderPage($cLoc,$nLeft=0,$nTop=0,$nWidth=0,$nHeight=0){
  if($cLoc !== ""){
    ...
    if($this->nExport == '2') $objDrawing->setWorksheet($this->objPHPExcel->getActiveSheet());
  }
}
