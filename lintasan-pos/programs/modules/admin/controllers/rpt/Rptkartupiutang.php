...
class Rptkartupiutang extends Bismillah_Controller{
    ...
    public function loadgrid(){
        ...
        $vdb    = $this->rptkartupiutang_m->loadgrid($va) ;
        $dbd    = $vdb['db'] ;
        $tglKemarin = date("Y-m-d",strtotime($va['tglAwal'])-(60*60*24));
        $saldo = $this->perhitungan_m->GetSaldoAkhirPiutang($va['customer'],$tglKemarin) ; ;
        $n = 0 ;
        $vare[] = array("no"=>"","faktur"=>"","tgl"=>"","keterangan"=>"Saldo Awal","debet"=>"","kredit"=>"","saldo"=>$saldo);
        $totdebet = 0 ;
        $totkredit = 0 ;
        while( $dbr = $this->rptkartupiutang_m->getrow($dbd) ){
            $n++;
    ...
    public function pilihcustomer(){
        $va 	= $this->input->post() ;
        $kode 	= $va['kode'] ;
        $data = $this->rptkartupiutang_m->getdata($kode) ;
        if(!empty($data)){
            echo('
            with(bos.rptkartupiutang.obj){
               find("#customer").val("'.$data['kode'].'") ;
               find("#namacustomer").val("'.$data['nama'].'");
               bos.rptkartupiutang.loadmodelcustomer("hide");
            }
    ...
    public function loadgrid2(){
        $va     = json_decode($this->input->post('request'), true) ;
        $vare   = array() ;
        $vdb    = $this->rptkartupiutang_m->loadgrid2($va) ;
        $dbd    = $vdb['db'] ;
        while( $dbr = $this->rptkartupiutang_m->getrow($dbd) ){
            $vaset   = $dbr ;
            $vaset['cmdpilih']    = '<button type="button" onClick="bos.rptkartupiutang.cmdpilih(\''.$dbr['kode'].'\')"
