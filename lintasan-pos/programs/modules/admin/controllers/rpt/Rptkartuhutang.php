<?php
class Rptkartuhutang extends Bismillah_Controller{
    protected $bdb ;
    protected $ss ;
    protected $abc ;
    public function __construct(){
        ...
        $this->load->model('rpt/rptkartuhutang_m') ;
        $this->load->model('func/perhitungan_m') ;
        $this->bdb = $this->rptkartuhutang_m ;
        $this->ss  = "ssrptkartuhutang_" ;
    }

    public function index(){
        $d    = array("setdate"=>date_set()) ;
        $this->load->view('rpt/rptkartuhutang', $d) ;
    }
    ...
    public function loadgrid(){
        ...
        $vdb    = $this->rptkartuhutang_m->loadgrid($va) ;
        $dbd    = $vdb['db'] ;
        $tglKemarin = date("Y-m-d",strtotime($va['tglAwal'])-(60*60*24));
        $saldo = $this->perhitungan_m->GetSaldoAkhirHutang($va['supplier'],$tglKemarin) ; ;
        $n = 0 ;
        $vare[] = array("no"=>"","faktur"=>"","tgl"=>"","keterangan"=>"Saldo Awal","debet"=>"","kredit"=>"","saldo"=>$saldo);
        $totdebet = 0 ;
        $totkredit = 0 ;
        while( $dbr = $this->rptkartuhutang_m->getrow($dbd) ){
            $n++;
    ...
    public function pilihsupplier(){
        $va   = $this->input->post() ;
        $kode   = $va['kode'] ;
        $data = $this->rptkartuhutang_m->getdata($kode) ;
        if(!empty($data)){
            echo('
            with(bos.rptkartuhutang.obj){
               find("#supplier").val("'.$data['kode'].'") ;
               find("#namasupplier").val("'.$data['nama'].'");
               bos.rptkartuhutang.loadmodelsupplier("hide");
            }
    ...
    public function loadgrid2(){
        $va     = json_decode($this->input->post('request'), true) ;
        $vare   = array() ;
        $vdb    = $this->rptkartuhutang_m->loadgrid2($va) ;
        $dbd    = $vdb['db'] ;
        while( $dbr = $this->rptkartuhutang_m->getrow($dbd) ){
            $vaset   = $dbr ;
            $vaset['cmdpilih']    = '<button type="button" onClick="bos.rptkartuhutang.cmdpilih(\''.$dbr['kode'].'\')"
    ...
    $vaset['kredit'] = string_2s($vaset['kredit']);
      ...
      savesession($this, "rptkartuhutang_rpt", json_encode($vare)) ;
      echo(' bos.rptkartuhutang.openreport() ; ') ;
  }

  public function showreport(){
      $va   = json_decode(getsession($this, $this->ss . "va", "{}"), true) ;
      $data = getsession($this,"rptkartuhutang_rpt") ;      
    ...
    $ketsupplier = $this->bdb->getval("nama", "kode = '{$va['supplier']}'", "supplier");
        ...
        $this->bospdf->ezText("<b>LAPORAN KARTU HUTANG</b>",$font+4,array("justification"=>"center")) ;
