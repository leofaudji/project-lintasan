...
class Rptsaldouangmuka extends Bismillah_Controller{
    protected $bdb ;
    protected $ss ;
    protected $abc ;
    public function __construct(){
        ...
        $this->load->model('rpt/rptsaldouangmuka_m') ;
        $this->load->model('func/perhitungan_m') ;
        $this->bdb = $this->rptsaldouangmuka_m ;
        $this->ss  = "ssrptsaldouangmuka_" ;
    }

    public function index(){
        $d    = array("setdate"=>date_set()) ;
        $this->load->view('rpt/rptsaldouangmuka', $d) ;
    }
    ...
    public function loadgrid(){
        ...
        $vdb    = $this->rptsaldouangmuka_m->loadgrid($va) ;
        $dbd    = $vdb['db'] ;
        $n = 0 ;
		$totsaldo = 0 ;
        while( $dbr = $this->rptsaldouangmuka_m->getrow($dbd) ){
            $n++;
            $vaset = $dbr;
            $vaset['saldo'] = $this->perhitungan_m->GetSaldoAkhirPiutang($dbr['kode'],$va['tgl'],"","U")  ;
	...
	public function initreport(){
      ...
      while( $dbr = $this->rptsaldouangmuka_m->getrow($dbd) ){
		$n++;
		$saldo = $this->perhitungan_m->GetSaldoAkhirPiutang($dbr['kode'],$va['tgl'],"","U") ; 
        
        $vare[]     = array("no"=>$n,"nama"=>$dbr['nama'],"alamat"=>$dbr['alamat'],"saldo"=>string_2s($saldo)) ;
      }
	  
      savesession($this, "rptsaldouangmuka_rpt", json_encode($vare)) ;
      echo(' bos.rptsaldouangmuka.openreport() ; ') ;
	}

	public function showreport(){
      $va   = json_decode(getsession($this, $this->ss . "va", "{}"), true) ;
      $data = getsession($this,"rptsaldouangmuka_rpt") ;      
      ...
      if(!empty($data)){ 
        ...
        $this->bospdf->ezText("<b>LAPORAN SALDO UANG MUKA</b>",$font+4,array("justification"=>"center")) ;
