...
class Rptsaldohutang extends Bismillah_Controller{
    ...
    public function loadgrid(){
        ...
        while( $dbr = $this->rptsaldohutang_m->getrow($dbd) ){
            $n++;
            $vaset = $dbr;
            $vaset['saldo'] = $this->perhitungan_m->GetSaldoAkhirHutang($dbr['kode'],$va['tgl']) ; 
            $vaset['no'] = $n;
	...
	public function initreport(){
      ...
      while( $dbr = $this->rptsaldohutang_m->getrow($dbd) ){
		$n++;
		$saldo = $this->perhitungan_m->GetSaldoAkhirHutang($dbr['kode'],$va['tgl']) ; 
	...
	public function showreport(){
      ...
      if(!empty($data)){ 
        ...
        $this->bospdf->ezText("<b>LAPORAN SALDO HUTANG</b>",$font+4,array("justification"=>"center")) ;
