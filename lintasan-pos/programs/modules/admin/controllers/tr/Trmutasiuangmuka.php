<?php
class Trmutasiuangmuka extends Bismillah_Controller{
    private $bdb ;
    public function __construct(){
        parent::__construct() ;
        $this->load->model('tr/trmutasiuangmuka_m') ;
        $this->load->model('func/updtransaksi_m') ;
        $this->load->helper('bdate');
        $this->bdb = $this->trmutasiuangmuka_m ;
    }

    public function index(){
        $d    = array("dnow"=>date("d-m-Y"), "setdate"=>date_set()) ;
        $this->load->view('tr/trmutasiuangmuka',$d) ;
    }
    ...
    public function loadgrid(){
        ...
        $vdb                = $this->trmutasiuangmuka_m->loadgrid($va) ;
        $dbd                = $vdb['db'] ;
        while( $dbr = $this->trmutasiuangmuka_m->getrow($dbd) ){
            $vaset                  = $dbr ;
            $vaset['tgl']           = date_2d($vaset['tgl']);
            $vaset['cmdedit']       = '<button type="button" onClick="bos.trmutasiuangmuka.cmdedit(\''.$dbr['faktur'].'\')"
                                        class="btn btn-success btn-grid">Edit</button>' ;
            $vaset['cmddelete']     = '<button type="button" onClick="bos.trmutasiuangmuka.cmddelete(\''.$dbr['faktur'].'\')"
                                        class="btn btn-danger btn-grid">Delete</button>' ;
            $vaset['cmdcetak']    = '<button type="button" onClick="bos.trmutasiuangmuka.cmdcetak(\''.$dbr['faktur'].'\')"
    ...
    public function init(){
        savesession($this, "ssmutasiuangmuka_faktur", "") ;
    }

    public function saving(){
        $va 	     = $this->input->post() ;
        $kode 	     = getsession($this, "ssmutasiuangmuka_faktur") ;
        $va['tgl']   = date_2s($va['tgl']) ;

        $this->trmutasiuangmuka_m->saving($kode, $va) ;
        echo(' bos.trmutasiuangmuka.settab(0) ;  ') ;
    }

    public function editing(){
        $va 	= $this->input->post() ;
        $faktur = $va['faktur'] ;
        $data   = $this->trmutasiuangmuka_m->getdatatotal($faktur) ;
        if(!empty($data)){
            savesession($this, "ssmutasiuangmuka_faktur", $faktur) ;
            $kodetransaksi[] = array("id"=>$data['kodetransaksi'],"text"=>$data['kodetransaksi']."-".$data['ketkdtr']);
            $customer[] = array("id"=>$data['customer'],"text"=>$data['customer']."-".$data['namacustomer']);

            echo('
            w2ui["bos-form-trmutasiuangmuka_grid2"].clear();
            with(bos.trmutasiuangmuka.obj){
         ...
         ') ;
            ...
            //loadgrid detail
            $vare = array();
            $dbd = $this->trmutasiuangmuka_m->getdatadetail($faktur) ;
            $n = 0 ;
            while( $dbr = $this->trmutasiuangmuka_m->getrow($dbd) ){
                $n++;
                $vaset   = $dbr ;
                $vaset['recid'] = $n;
                $vaset['no'] = $n;
                $vaset['cmddelete'] = '<button type="button" onClick="bos.trmutasiuangmuka.grid2_deleterow('.$n.')"
                                        class="btn btn-danger btn-grid">Delete</button>' ;
                $vaset['cmddelete']	= html_entity_decode($vaset['cmddelete']) ;
            ...
            echo('
                w2ui["bos-form-trmutasiuangmuka_grid2"].add('.$vare.');
                bos.trmutasiuangmuka.initdetail();
                bos.trmutasiuangmuka.settab(1) ;
            ');
        }
    }
    public function deleting(){
        $va 	= $this->input->post() ;
        $this->trmutasiuangmuka_m->deleting($va['faktur']) ;
        echo('bos.trmutasiuangmuka.grid1_reloaddata() ; ') ;

    }

    public function seekrekening(){
        $search     = $this->input->get('q');
        $vdb    = $this->trmutasiuangmuka_m->seekrekening($search) ;
        $dbd    = $vdb['db'] ;
        $vare   = array();
        while( $dbr = $this->trmutasiuangmuka_m->getrow($dbd) ){
    ...
    public function seekcustomer(){
        $search     = $this->input->get('q');
        $vdb    = $this->trmutasiuangmuka_m->seekcustomer($search) ;
        $dbd    = $vdb['db'] ;
        $vare   = array();
        while( $dbr = $this->trmutasiuangmuka_m->getrow($dbd) ){
    ...
    public function seekkodetransaksi(){
        $search     = $this->input->get('q');
        $vdb    = $this->trmutasiuangmuka_m->seekkodetransaksi($search) ;
        $dbd    = $vdb['db'] ;
        $vare   = array();
        while( $dbr = $this->trmutasiuangmuka_m->getrow($dbd) ){
            $vare[] 	= array("id"=>$dbr['kode'], "text"=>$dbr['kode'] . " - " . $dbr['keterangan']) ;
    ...
    public function getfaktur(){
        $va 	= $this->input->post() ;
        $faktur  = $this->trmutasiuangmuka_m->getfaktur(FALSE) ;

        echo('
        bos.trmutasiuangmuka.obj.find("#faktur").val("'.$faktur.'") ;
        ') ;
    }
    ...
    public function printbuktibm(){
        ...
        if(!empty($dbr)){
            ...
            bos.trmutasiuangmuka.printbukti("'.$html.'");
            ');
        }else{
