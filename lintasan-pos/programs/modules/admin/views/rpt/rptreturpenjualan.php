...
<div class="header active">
    <table class="header-table">
        <tr>
            ...
            <td class="title">Laporan Retur Penjualan</td>
            <td class="button">
                <table class="header-button" align="right">
                    <tr> 
                        <td>
                            <div class="btn-circle btn-close transition" onclick="bos.rptreturpenjualan.close()">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="wm-title">Preview Detail Retur Penjualan</h4>
                    </div>
...
<script type="text/javascript">
    ...
    bos.rptreturpenjualan.grid1_data    = null ;
    bos.rptreturpenjualan.grid1_loaddata= function(){
        var tglAwal = bos.rptreturpenjualan.obj.find("#tglawal").val();
        var tglAkhir = bos.rptreturpenjualan.obj.find("#tglakhir").val();
        this.grid1_data 		= {'tglAwal':tglAwal,'tglAkhir':tglAkhir} ;
    }

    bos.rptreturpenjualan.grid1_load    = function(){
        this.obj.find("#grid1").w2grid({
            name	: this.id + '_grid1',
            limit 	: 100 ,
            url 	: bos.rptreturpenjualan.base_url + "/loadgrid",
    ...
    bos.rptreturpenjualan.grid1_setdata	= function(){
        w2ui[this.id + '_grid1'].postData 	= this.grid1_data ;
    }

    bos.rptreturpenjualan.grid1_reload		= function(){
        w2ui[this.id + '_grid1'].reload() ;
    }

    bos.rptreturpenjualan.grid1_destroy 	= function(){
        if(w2ui[this.id + '_grid1'] !== undefined){
            w2ui[this.id + '_grid1'].destroy() ;
        }
    }

    bos.rptreturpenjualan.grid1_render 	= function(){
        this.obj.find("#grid1").w2render(this.id + '_grid1') ;
    }

    bos.rptreturpenjualan.grid1_reloadData	= function(){
        this.grid1_loaddata() ;
        this.grid1_setdata() ;
        this.grid1_reload() ;
    }
    ...
    bos.rptreturpenjualan.cmdpreviewdetail	= function(faktur){
        bjs.ajax(this.url + '/PreviewDetail', 'faktur=' + faktur);
    }

    bos.rptreturpenjualan.cmdrefresh          = bos.rptreturpenjualan.obj.find("#cmdrefresh") ;
    bos.rptreturpenjualan.cmdpreview     = bos.rptreturpenjualan.obj.find("#cmdpreview")
    bos.rptreturpenjualan.cmdCetakLaporanDetail     = bos.rptreturpenjualan.obj.find("#cmdCetakLaporanDetail") ;
    bos.rptreturpenjualan.initfunc    = function(){
        this.obj.find("#cmdpreview").on("click", function(e){
            e.preventDefault() ;
            bos.rptreturpenjualan.initreportTotal(0,0) ;
        }) ;

        this.obj.find("#cmdrefresh").on("click", function(e){
            e.preventDefault() ;
            bos.rptreturpenjualan.grid1_reloadData() ;
        }) ;
        this.obj.find("#cmdCetakLaporanDetail").on("click",function(e){
            e.preventDefault() ;
            bos.rptreturpenjualan.initReportDetailPembelian(0,0) ;
        });
    }

    bos.rptreturpenjualan.initReportDetailPembelian  = function(s,e){
        bjs.ajax(this.base_url+ '/initreport', bjs.getdataform(this.obj.find("form"))) ;
    }

    bos.rptreturpenjualan.loadmodalpreview      = function(l){
        this.obj.find("#wrap-preview-detail-d").modal(l) ;
    }

    bos.rptreturpenjualan.initcomp		= function(){
        this.grid1_loaddata() ;
        this.grid1_load() ;
        this.grid2_load() ;
    ...
    bos.rptreturpenjualan.initcallback	= function(){
        this.obj.on('remove', function(){
            bos.rptreturpenjualan.grid1_destroy() ;
            bos.rptreturpenjualan.grid2_destroy() ;
            bos.rptreturpenjualan.grid2_destroy() ;
        }) ;
    }

    bos.rptreturpenjualan.setPreview      = function(cFaktur){
        bos.rptreturpenjualan.initreport(0,0) ;
    }

    bos.rptreturpenjualan.initreport  = function(s,e){
        bjs.ajax(this.base_url+ '/initreport', bjs.getdataform(this.obj.find("form"))) ;
    }

    bos.rptreturpenjualan.initreportTotal  = function(s,e){
        bjs.ajax(this.base_url+ '/initreportTotal', bjs.getdataform(this.obj.find("form"))) ;
    }

    bos.rptreturpenjualan.openreport  = function(){
        bjs_os.form_report( this.base_url + '/showreport' ) ;
    }

    bos.rptreturpenjualan.openreporttotal  = function(){
        bjs_os.form_report( this.base_url + '/showreporttotal' ) ;
    }

    bos.rptreturpenjualan.grid2_data    = null ;
    bos.rptreturpenjualan.grid2_loaddata= function(){
        this.grid2_data         = {} ;
    }


    bos.rptreturpenjualan.grid2_load    = function(){
    ...
    bos.rptreturpenjualan.grid2_reload     = function(){
        w2ui[this.id + '_grid2'].reload() ;
    }
    bos.rptreturpenjualan.grid2_destroy    = function(){
        if(w2ui[this.id + '_grid2'] !== undefined){
            w2ui[this.id + '_grid2'].destroy() ;
        }
    }

    bos.rptreturpenjualan.grid2_render     = function(){
        this.obj.find("#grid2").w2render(this.id + '_grid2') ;
    }

    bos.rptreturpenjualan.grid2_reloaddata = function(){
        this.grid2_reload() ;
    }

    $(function(){
        bos.rptreturpenjualan.initcomp() ;
        bos.rptreturpenjualan.initcallback() ;
        bos.rptreturpenjualan.initfunc() ;
    });
</script>