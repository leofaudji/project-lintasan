...
<div class="header active">
    <table class="header-table">
        <tr>
            ...
            <td class="title">Laporan Kartu Persekot</td>
            <td class="button">
                <table class="header-button" align="right">
                    <tr> 
                        <td>
                            <div class="btn-circle btn-close transition" onclick="bos.rptkartupersekot.close()">
...
<script type="text/javascript">
    ...
    bos.rptkartupersekot.grid1_data    = null ;
    bos.rptkartupersekot.grid1_loaddata= function(){
        var tglAwal = bos.rptkartupersekot.obj.find("#tglawal").val();
        var tglAkhir = bos.rptkartupersekot.obj.find("#tglakhir").val();
        var supplier = bos.rptkartupersekot.obj.find("#supplier").val();
        this.grid1_data 		= {'tglAwal':tglAwal,'tglAkhir':tglAkhir,'supplier':supplier} ;
    }

    bos.rptkartupersekot.grid1_load    = function(){
        this.obj.find("#grid1").w2grid({
            name	: this.id + '_grid1',
            limit 	: 100 ,
            url 	: bos.rptkartupersekot.base_url + "/loadgrid",
    ...
    bos.rptkartupersekot.grid1_setdata	= function(){
        w2ui[this.id + '_grid1'].postData 	= this.grid1_data ;
    }

    bos.rptkartupersekot.grid1_reload		= function(){
        w2ui[this.id + '_grid1'].reload() ;
    }

    bos.rptkartupersekot.grid1_destroy 	= function(){
        if(w2ui[this.id + '_grid1'] !== undefined){
            w2ui[this.id + '_grid1'].destroy() ;
        }
    }

    bos.rptkartupersekot.grid1_render 	= function(){
        this.obj.find("#grid1").w2render(this.id + '_grid1') ;
    }

    bos.rptkartupersekot.grid1_reloadData	= function(){
        this.grid1_loaddata() ;
        this.grid1_setdata() ;
        this.grid1_reload() ;
    }
    ...
    //grid2 daftarstock
    bos.rptkartupersekot.grid2_data    = null ;
    bos.rptkartupersekot.grid2_loaddata= function(){
        this.grid2_data 		= {} ;
    }

    bos.rptkartupersekot.grid2_load    = function(){
        this.obj.find("#grid2").w2grid({
            name	: this.id + '_grid2',
            limit 	: 100 ,
            url 	: bos.rptkartupersekot.base_url + "/loadgrid2",
    ...
    bos.rptkartupersekot.grid2_setdata	= function(){
        w2ui[this.id + '_grid2'].postData 	= this.grid2_data ;
    }
    bos.rptkartupersekot.grid2_reload		= function(){
        w2ui[this.id + '_grid2'].reload() ;
    }
    bos.rptkartupersekot.grid2_destroy 	= function(){
        if(w2ui[this.id + '_grid2'] !== undefined){
            w2ui[this.id + '_grid2'].destroy() ;
        }
    }

    bos.rptkartupersekot.grid2_render 	= function(){
        this.obj.find("#grid2").w2render(this.id + '_grid2') ;
    }

    bos.rptkartupersekot.grid2_reloaddata	= function(){
        this.grid2_loaddata() ;
        this.grid2_setdata() ;
        this.grid2_reload() ;
    }
    
    bos.rptkartupersekot.cmdpilih 		= function(kode){
        bjs.ajax(this.url + '/pilihsupplier', 'kode=' + kode);
    }
	
	bos.rptkartupersekot.obj.find("#cmdview").on("click", function(){
		bos.rptkartupersekot.initreport();
	}) ;

    bos.rptkartupersekot.initreport  = function(s,e){
        bjs.ajax(this.base_url+ '/initreport', bjs.getdataform(this.obj.find("form"))) ;
    }
    bos.rptkartupersekot.openreport  = function(){
        bjs_os.form_report( this.base_url + '/showreport' ) ;
    }
	
    bos.rptkartupersekot.cmdrefresh          = bos.rptkartupersekot.obj.find("#cmdrefresh") ;
    bos.rptkartupersekot.initfunc    = function(){
        this.obj.find("#cmdrefresh").on("click", function(e){
            e.preventDefault() ;
            bos.rptkartupersekot.grid1_reloadData() ;
        }) ;
        
        this.obj.find("#cmdsupplier").on("click", function(e){
            bos.rptkartupersekot.loadmodelsupplier("show");
            bos.rptkartupersekot.grid2_reloaddata() ;
        }) ;
        
        this.obj.find("#supplier").on("blur", function(e){
            if(bos.rptkartupersekot.obj.find("#supplier").val() !== ""){
                var supplier = bos.rptkartupersekot.obj.find("#supplier").val();
                bjs.ajax( bos.rptkartupersekot.base_url + '/seeksupplier', bjs.getdataform(this)) ;
            }
        });

    }

    bos.rptkartupersekot.initcomp		= function(){
        this.grid1_loaddata() ;
        this.grid1_load() ;
        this.grid2_load() ;
    ...
    bos.rptkartupersekot.loadmodelsupplier      = function(l){
        this.obj.find("#wrap-pencariansupplier-d").modal(l) ;
    }
    bos.rptkartupersekot.initcallback	= function(){
        this.obj.on('remove', function(){
            bos.rptkartupersekot.grid1_destroy() ;
            bos.rptkartupersekot.grid2_destroy() ;
        }) ;
    }

    $(function(){
        bos.rptkartupersekot.initcomp() ;
        bos.rptkartupersekot.initcallback() ;
        bos.rptkartupersekot.initfunc() ;
    });
</script>