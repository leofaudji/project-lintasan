...
<div class="header active">
    <table class="header-table">
        <tr>
            ...
            <td class="title">Laporan Hasil Produksi</td>
            <td class="button">
                <table class="header-button" align="right">
                    <tr> 
                        <td>
                            <div class="btn-circle btn-close transition" onclick="bos.rpthasilproduksi.close()">
...
<script type="text/javascript">
    ...
    bos.rpthasilproduksi.grid1_data    = null ;
    bos.rpthasilproduksi.grid1_loaddata= function(){
        var tglAwal = bos.rpthasilproduksi.obj.find("#tglawal").val();
        var tglAkhir = bos.rpthasilproduksi.obj.find("#tglakhir").val();
        this.grid1_data 		= {'tglAwal':tglAwal,'tglAkhir':tglAkhir} ;
    }

    bos.rpthasilproduksi.grid1_load    = function(){
        this.obj.find("#grid1").w2grid({
            name	: this.id + '_grid1',
            limit 	: 100 ,
            url 	: bos.rpthasilproduksi.base_url + "/loadgrid",
    ...
    bos.rpthasilproduksi.grid1_setdata	= function(){
        w2ui[this.id + '_grid1'].postData 	= this.grid1_data ;
    }

    bos.rpthasilproduksi.grid1_reload		= function(){
        w2ui[this.id + '_grid1'].reload() ;
    }

    bos.rpthasilproduksi.grid1_destroy 	= function(){
        if(w2ui[this.id + '_grid1'] !== undefined){
            w2ui[this.id + '_grid1'].destroy() ;
        }
    }

    bos.rpthasilproduksi.grid1_render 	= function(){
        this.obj.find("#grid1").w2render(this.id + '_grid1') ;
    }

    bos.rpthasilproduksi.grid1_reloadData	= function(){
        this.grid1_loaddata() ;
        this.grid1_setdata() ;
        this.grid1_reload() ;
    }
    ...
    bos.rpthasilproduksi.cmdpreviewdetail	= function(faktur){
        bjs.ajax(this.url + '/PreviewDetail', 'faktur=' + faktur);
    }

    bos.rpthasilproduksi.cmdrefresh          = bos.rpthasilproduksi.obj.find("#cmdrefresh") ;
    bos.rpthasilproduksi.cmdpreview     = bos.rpthasilproduksi.obj.find("#cmdpreview")
    bos.rpthasilproduksi.cmdCetakLaporanDetail     = bos.rpthasilproduksi.obj.find("#cmdCetakLaporanDetail") ;
    bos.rpthasilproduksi.initfunc    = function(){
        this.obj.find("#cmdpreview").on("click", function(e){
            e.preventDefault() ;
            bos.rpthasilproduksi.initreportTotal(0,0) ;
        }) ;

        this.obj.find("#cmdrefresh").on("click", function(e){
            e.preventDefault() ;
            bos.rpthasilproduksi.grid1_reloadData() ;
        }) ;
        this.obj.find("#cmdCetakLaporanDetail").on("click",function(e){
            e.preventDefault() ;
            bos.rpthasilproduksi.initReportDetailPP(0,0) ;
        });
    }

    bos.rpthasilproduksi.initReportDetailPP  = function(s,e){
        bjs.ajax(this.base_url+ '/initreport', bjs.getdataform(this.obj.find("form"))) ;
    }

    bos.rpthasilproduksi.loadmodalpreview      = function(l){
        this.obj.find("#wrap-preview-detail-d").modal(l) ;
    }

    bos.rpthasilproduksi.initcomp		= function(){
        this.grid1_loaddata() ;
        this.grid1_load() ;
        this.grid2_load() ;
    ...
    bos.rpthasilproduksi.initcallback	= function(){
        this.obj.on('remove', function(){
            bos.rpthasilproduksi.grid1_destroy() ;
            bos.rpthasilproduksi.grid2_destroy() ;
            bos.rpthasilproduksi.grid2_destroy() ;
        }) ;
    }

    bos.rpthasilproduksi.setPreview      = function(cFaktur){
        bos.rpthasilproduksi.initreport(0,0) ;
    }

    bos.rpthasilproduksi.initreport  = function(s,e){
        bjs.ajax(this.base_url+ '/initreport', bjs.getdataform(this.obj.find("form"))) ;
    }

    bos.rpthasilproduksi.initreportTotal  = function(s,e){
        bjs.ajax(this.base_url+ '/initreportTotal', bjs.getdataform(this.obj.find("form"))) ;
    }

    bos.rpthasilproduksi.openreport  = function(){
        bjs_os.form_report( this.base_url + '/showreport' ) ;
    }

    bos.rpthasilproduksi.openreporttotal  = function(){
        bjs_os.form_report( this.base_url + '/showreporttotal' ) ;
    }

    bos.rpthasilproduksi.grid2_data    = null ;
    bos.rpthasilproduksi.grid2_loaddata= function(){
        this.grid2_data         = {} ;
    }


    bos.rpthasilproduksi.grid2_load    = function(){
        this.obj.find("#grid2").w2grid({
            ...
            columns: [
                ...
                { field: 'qty', render: 'float:2', caption: 'Qty', size: '40px', sortable: false, style:'text-align:right'},
    ...
    bos.rpthasilproduksi.grid2_reload     = function(){
        w2ui[this.id + '_grid2'].reload() ;
    }
    bos.rpthasilproduksi.grid2_destroy    = function(){
        if(w2ui[this.id + '_grid2'] !== undefined){
            w2ui[this.id + '_grid2'].destroy() ;
        }
    }

    bos.rpthasilproduksi.grid2_render     = function(){
        this.obj.find("#grid2").w2render(this.id + '_grid2') ;
    }

    bos.rpthasilproduksi.grid2_reloaddata = function(){
        this.grid2_reload() ;
    }

    $(function(){
        bos.rpthasilproduksi.initcomp() ;
        bos.rpthasilproduksi.initcallback() ;
        bos.rpthasilproduksi.initfunc() ;
    });
</script>