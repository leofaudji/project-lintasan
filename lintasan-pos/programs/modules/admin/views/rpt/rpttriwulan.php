<div class="header active">
    <table class="header-table">
        <tr>
            ...
            <td class="title">
                <div class="nav ">
                    <div class="btn-group" id="trtriw">
                        <button class="btn btn-tab tpel active" href="#trtriw_1" data-toggle="tab" >Laporan Triwulan</button>
                        <button class="btn btn-tab tpel" href="#trtriw_2" data-toggle="tab">Setup</button>
                    </div>
                </div>
            </td>
            <td class="button">
                <table class="header-button" align="right">
                    <tr> 
                        <td>
                            <div class="btn-circle btn-close transition" onclick="bos.rpttriwulan.close()">
    ...
    <form novalidate>
        <div class="bodyfix scrollme" style="height:100%">
            <div class="tab-content full-height">
                <div role="tabpanel" class="tab-pane active full-height" id="trtriw_1" style="padding-top:5px;">
                    <table width="100%">
                        <tr>
                            <td height="25px" width="100%">
                                <table class="osxtable form">
                                    <tr>
                                        <td width="85px">
                                            <input style="width:80px" type="text" class="form-control date" id="periode" name="periode" required value=<?=date("m-Y")?> <?=date_periodset(true)?>></td>
                ...
                <div role="tabpanel" class="tab-pane fade full-height" id="trtriw_2">
...
<script type="text/javascript">
    ...
    bos.rpttriwulan.grid1_data 	 = null ;
    bos.rpttriwulan.grid1_loaddata= function(){
        this.grid1_data 		= {
            "periode"	   : this.obj.find("#periode").val()
        } ;
    }

    bos.rpttriwulan.grid1_load    = function(){
        var periode = this.obj.find("#periode").val();
        var arrperiod = periode.split("-");
        var kolom = [];
        ...
        var arrtriwulan = ["I","II","III","IV"];

        for(t=1;t>=0;t--){
            var bulan = arrperiod[0]-(t*3);
            var thn = arrperiod[1];

            if(bulan < 1){
                var blnthn = ((bulan * -1) / 12) + 1;
                blnthn = Math.floor(blnthn);
                thn = thn - blnthn;
                bulan = 12+bulan;
            }

            var frtriwulan = (bulan / 3);
            frtriwulan = Math.ceil(frtriwulan) ;

            var head  = "Triwulan "+arrtriwulan[frtriwulan-1]+" "+thn;
            var key = arrtriwulan[frtriwulan-1]+"-"+thn;
            kolom.push({ field:key,caption:head,  size: '150px',sortable: false,style:'text-align:right'});
        }
        kolom.push({ field: 'pers', caption: '%', size: '80px', sortable: false,style:'text-align:right'});
        ...
        this.obj.find("#grid1").w2grid({
            name		: this.id + '_grid1',
            limit 	: 100 ,
            url 		: bos.rpttriwulan.base_url + "/loadgrid",
    ...
    bos.rpttriwulan.grid1_setdata	= function(){
        w2ui[this.id + '_grid1'].postData 	= this.grid1_data ;
    }
    bos.rpttriwulan.grid1_reload		= function(){
        w2ui[this.id + '_grid1'].reload() ;
    }
    bos.rpttriwulan.grid1_destroy 	= function(){
        if(w2ui[this.id + '_grid1'] !== undefined){
            w2ui[this.id + '_grid1'].destroy() ;
        }
    }

    bos.rpttriwulan.grid1_removeallcolumn 	= function(){
    ...
    bos.rpttriwulan.grid1_initcolumn 	= function(){
        //bos.rpttriwulan.grid1_removeallcolumn();
    ...
    bos.rpttriwulan.grid1_render 	= function(){
        this.obj.find("#grid1").w2render(this.id + '_grid1') ;
    }

    bos.rpttriwulan.grid1_reloaddata	= function(){
        this.grid1_loaddata() ;
        this.grid1_setdata() ;
        this.grid1_reload() ;
    }

    bos.rpttriwulan.settab 		= function(n){
        this.obj.find("#trtriw button:eq("+n+")").tab("show") ;
    }

    bos.rpttriwulan.tabsaction	= function(n){
        if(n == 0){
            this.obj.find(".bodyfix").css("height","100%") ;
            this.obj.find(".footer").addClass("hidden") ;
            bos.rpttriwulan.grid1_render() ;
        }else{
    ...
    bos.rpttriwulan.obj.find("#cmdview").on("click", function(){
        bos.rpttriwulan.initreport();
    }) ;

    bos.rpttriwulan.initreport  = function(s,e){
        bjs.ajax(this.base_url+ '/initreport', bjs.getdataform(this.obj.find("form"))) ;
    }
    bos.rpttriwulan.openreport  = function(){
        bjs_os.form_report( this.base_url + '/showreport' ) ;
    }


    bos.rpttriwulan.initcomp		= function(){
    ...
    bos.rpttriwulan.initcallback	= function(){
        this.obj.on("bos:tab", function(e){
            bos.rpttriwulan.tabsaction( e.i )  ;
        });
        this.obj.on('remove', function(){
            bos.rpttriwulan.grid1_destroy() ;
        }) ;
    }

    bos.rpttriwulan.objs = bos.rpttriwulan.obj.find("#cmdsave") ;
    bos.rpttriwulan.initfunc	   = function(){
        this.grid1_loaddata() ;
        this.grid1_load() ;
        this.obj.find('form').on("submit", function(e){
            ...
            if( bjs.isvalidform(this) ){
                bjs.ajax( bos.rpttriwulan.base_url + '/saving', bjs.getdataform(this) , bos.rpttriwulan.cmdsave) ;
            }
            // });
        ...
        this.obj.find("#cmdrefresh").on("click", function(e){
            //bos.rpttriwulan.grid1_reloaddata();
            bos.rpttriwulan.grid1_initcolumn();
        });
    }

    $(".selectrekakt").select2({
        ajax: {
            url: bos.rpttriwulan.base_url + '/seekrekening',
            dataType: 'json',
            delay: 250,
    ...
    $(function(){
        bos.rpttriwulan.initcomp() ;
        bos.rpttriwulan.initcallback() ;
        bos.rpttriwulan.initfunc() ;
    });
</script>
