...
<script type="text/javascript">
    ...
    bos.trpr.grid2_append    = function(no,kode,keterangan,qty,satuan){
    ...
    bos.trpr.grid2_deleterow = function(recid){
        if(confirm("Item di hapus dari detail pembelian???"+recid)){
            ...
            bos.trpr.grid2_urutkan();
            bos.trpr.hitungsubtotal();
        }
    }

    bos.trpr.grid2_urutkan = function(){
        var datagrid = w2ui[this.id + '_grid2'].records;
    ...
    //grid3 daftarstock
    bos.trpr.grid3_data    = null ;
    bos.trpr.grid3_loaddata= function(){
        this.grid3_data 		= {} ;
    }

    bos.trpr.grid3_load    = function(){
        this.obj.find("#grid3").w2grid({
            name	: this.id + '_grid3',
            limit 	: 100 ,
            url 	: bos.trpr.base_url + "/loadgrid3",
    ...
    bos.trpr.grid3_setdata	= function(){
        w2ui[this.id + '_grid3'].postData 	= this.grid3_data ;
    }
    bos.trpr.grid3_reload		= function(){
        w2ui[this.id + '_grid3'].reload() ;
    }
    bos.trpr.grid3_destroy 	= function(){
        if(w2ui[this.id + '_grid3'] !== undefined){
            w2ui[this.id + '_grid3'].destroy() ;
        }
    }

    bos.trpr.grid3_render 	= function(){
        this.obj.find("#grid3").w2render(this.id + '_grid3') ;
    }

    bos.trpr.grid3_reloaddata	= function(){
        this.grid3_loaddata() ;
        this.grid3_setdata() ;
        this.grid3_reload() ;
    }

    bos.trpr.cmdpilih 		= function(kode){
    ...
    bos.trpr.initfunc	   = function(){
        this.obj.find("#cmdok").on("click", function(e){
            ...
            bos.trpr.grid2_append(no,stock,keterangan,qty,satuan);
        }) ;

        this.obj.find("#cmdstock").on("click", function(e){
            bos.trpr.loadmodelstock("show");
            bos.trpr.grid3_reloaddata() ;
        }) ;

        this.obj.find("#nomor").on("blur", function(e){
            var no = bos.trpr.obj.find("#nomor").val();
            var datagrid = w2ui['bos-form-trpr_grid2'].records;
            if(no <= datagrid.length){
                bos.trpr.obj.find("#stock").val(w2ui['bos-form-trpr_grid2'].getCellValue(no-1,1));
                bos.trpr.obj.find("#namastock").val(w2ui['bos-form-trpr_grid2'].getCellValue(no-1,2));
                bos.trpr.obj.find("#qty").val(w2ui['bos-form-trpr_grid2'].getCellValue(no-1,3));
                bos.trpr.obj.find("#satuan").val(w2ui['bos-form-trpr_grid2'].getCellValue(no-1,4));
            }else{
                bos.trpr.obj.find("#nomor").val(datagrid.length + 1)
            }
        ...
        this.obj.find("#stock").on("blur", function(e){
            e.preventDefault ;
            var cKodeStock = bos.trpr.obj.find("#stock").val();
            bos.trpr.getDataStock(cKodeStock) ;
            $('#harga').focus();
        });
        ...
        this.obj.find('form').on("submit", function(e){
            ...
            if( bjs.isvalidform(this) ){
                ...
                bjs.ajax( bos.trpr.base_url + '/saving', bjs.getdataform(this)+"&grid2="+datagrid2 , bos.trpr.cmdsave) ;
            }
    ...
    $('#gudang').select2({
        ...
        ajax: {
            url: bos.trpr.base_url + '/seekgudang',
            dataType: 'json',
            delay: 250,
