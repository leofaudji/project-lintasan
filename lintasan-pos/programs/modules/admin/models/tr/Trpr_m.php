<?php
class Trpr_m extends Bismillah_Model{
    public function loadgrid($va){
        ...
        $dbd      = $this->select("pr_total t", $field, $where, $join, "", "t. faktur ASC", $limit) ;
        $dba      = $this->select("pr_total t", "t.id", $where) ;
    ...
    public function saving($faktur, $va){
        ...
        $this->update("pr_total", $data, $where, "") ;

        //insert detail po
        $vaGrid = json_decode($va['grid2']);
        $this->delete("pr_detail", "faktur = '{$va['faktur']}'" ) ;
        foreach($vaGrid as $key => $val){
            ...
            $vadetail = array("faktur"=>$va['faktur'],
                              "stock"=>$cKodeStock,
                              "qty"=>$val->qty);
            $this->insert("pr_detail",$vadetail);
        }
    }
    ...
    public function getdatatotal($faktur){
        ...
        $dbd   = $this->select("pr_total t", $field, $where, $join) ;
    ...
    public function deleting($faktur){
        $this->edit("pr_total",array("status"=>2),"faktur = " . $this->escape($faktur));
    }
