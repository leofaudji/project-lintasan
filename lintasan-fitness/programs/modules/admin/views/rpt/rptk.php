...
<div class="bodyfix" style="height: calc(100% - 34px) ;overflow-y:auto;">
	<table class="osxtable form" style="margin:0;">
		...
		<tr>
			<td width="15%" style="padding:0"><label for="retur_faktur">Lunas</label> </td>
			<td width="1%">:</td>
			<td colspan="2">
				...
				<div class="radio-inline">
					<label>
						<input type="radio" name="retur_faktur" value="0" checked> Belum
					</label>
				</div>
			</td>
		</tr>
...
<script type="text/javascript">
   <?=cekbosjs();?>
   bos.rptk.initreport  = function(s,e){
   ...
   bos.rptk.openreport  = function(){
      bos.rptk.cmdview.button('reset') ;
      bjs.initprogress(this.obj.find("#progress_1"),100,100) ;
      bjs_os.form_report( this.base_url + '/showreport' ) ;
   }

	bos.rptk.initcomp    = function(){
      bjs.initdate("#" + this.id + " .date") ;
   }

   bos.rptk.cmdview     = bos.rptk.obj.find("#cmdview")
   bos.rptk.initfunc    = function(){
      this.obj.find("form").on("submit", function(e){
         e.preventDefault() ;
         bos.rptk.cmdview.button('loading') ;
         bos.rptk.initreport(0,0) ;
      }) ;
   }
