...
class Trkasir extends Bismillah_Controller{
   ...
   public function loadgrid(){
		$brg 	= json_decode(getsession($this, "sstrkasir_barang", "{}"), true) ;
		$re 	= array() ;
		$tot  = array("qty"=>0, "tot"=>0) ;
		foreach ($brg as $key => $r) {
			$r['cmddel'] = html_entity_decode('<button type="button" onClick="bos.trkasir.cmddel(\''.$key.'\')"
	...
	public function index(){
		$data 	= array() ;
		$this->load->view("tr/trkasir") ;
	}

	public function init(){
		savesession($this, "sstrkasir_id", "") ;
		savesession($this, "sstrkasir_barang", "{}") ;
		//getsaldo kas
		$kas 	= $this->bdb->getkas(date("Y-m-d")) ;
		echo('
			bos.trkasir.obj.find("#faktur").html("'.$this->getfaktur(false).'") ;
			bos.trkasir.obj.find("#kas").html("Saldo Kas: Rp.'.number_format($kas).'") ;
		') ;
	}

	public function savepel(){
		$va 	= $this->input->post() ;
		savesession($this, "sstrkasir_id_pel", $va['id']) ;
		echo(' bos.trkasir.obj.find("#id_brg").select2("open") ;') ;
	}
	...
	public function addbarang(){
		...
		savesession($this, "sstrkasir_barang", json_encode($brg)) ;
		...
		echo(' bos.trkasir.brgdone() ; bos.trkasir.settotal('.$tot.') ; ') ;
	}
	...
	public function setharga(){
		...
		if($brg['stok'] > 0){
			...
			$harga = $brg['harga'];
			echo('
				bos.trkasir.brg_stok 	= '.$stok.' ;
				bos.trkasir.brg_harga  = '.$harga.' ;
				bos.trkasir.obj.find("#brg_harga").html("'.number_format($harga).'") ;
				bos.trkasir.obj.find("#brg_total").html("'.number_format($harga).'") ;
				bos.trkasir.obj.find("#brg_stok").html("'.number_format($stok).'") ;
				bos.trkasir.obj.find("#qty").focus() ;
			') ;
		}else{
			echo('
				alert("Stok Kosong") ;
   ...
   public function saving(){
		$va 	  = $this->input->post() ;
		$tgl 	  = date("Y-m-d") ;
		$id_pel = getsession($this, "sstrkasir_id_pel") ;
		$f 	  = $this->getfaktur() ;
		$brg 	  = json_decode(getsession($this, "sstrkasir_barang", "{}"), true) ;
		$total  = 0 ;
		//brg_jual
