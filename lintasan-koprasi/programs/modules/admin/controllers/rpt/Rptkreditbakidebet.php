...
class Rptkreditbakidebet extends Bismillah_Controller{ 
  ...
  public function loadgrid3(){
		...
		$dbd    = $vdb['db'] ;
		while( $dbr = $this->rptkreditbakidebet_m->getrow($dbd) ){
				$vaset   = $dbr ;
				$vaset['cmdpilih']    = '<button type="button" onClick="bos.rptkreditbakidebet.cmdpilih(\''.$dbr['rekening'].'\')"
	...
	public function pilih(){
		...
		if(!empty($data)){
		  echo('
				with(bos.rptkreditbakidebet.obj){ 
					find("#rekening").val("'.$data['rekening'].'") ; 
				}
				bos.rptkreditbakidebet.loadmodelstock("hide");
