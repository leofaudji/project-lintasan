<?php
class Rptkreditjadwal extends Bismillah_Controller{ 
  protected $bdb ; 
  public function __construct(){ 
    parent::__construct() ;
    $this->load->model("rpt/rptkreditjadwal_m") ;
    $this->load->model("func/kredit_m") ; 
    $this->bdb   = $this->rptkreditjadwal_m ;
  }  
 
  public function index(){
    $this->load->view("rpt/rptkreditjadwal") ; 
  ...
  public function loadgrid(){
    ...
    while( $dbr = $this->bdb->getrow($dbd) ){ 
      ...
      $dk = $this->kredit_m->simulasi_GetAngsuran($dbr['tgl'],$dbr['lama'],$dbr['plafond'],$dbr['sukubunga'],$dbr['caraperhitungan']) ;
      
      for($i=1;$i<=$dbr['lama'];$i++){
        $vs['tgl'] = date("d-M-Y",date_nextmonth(strtotime($dbr['tgl']),$i));
        $vs['no'] = ++$n ;
        $vs['pokok'] = string_2s($dk[$i]['Pokok']) ; 
        $vs['bunga'] = string_2s($dk[$i]['Bunga']) ;
        $vs['jumlah'] = string_2s($dk[$i]['Pokok'] + $dk[$i]['Bunga']) ;
        $vs['bakidebet'] = string_2s($dk[$i]['BakiDebet']) ;
        unset($vs['id']) ;  
        $vare[]    = $vs ;
        unset($vs['cmdcetak']) ;       
        $varpt[]  = $vs ;
      ...
      savesession($this, "rptkreditjadwal_rpt", json_encode($varpt)) ;   
  }

  public function init(){
    savesession($this, "ssrptkreditjadwal_id", "") ;     
  } 
  ...
  public function loadgrid3(){
		...
		$dbd    = $vdb['db'] ;
		while( $dbr = $this->rptkreditjadwal_m->getrow($dbd) ){
				$vaset   = $dbr ;
				$vaset['cmdpilih']    = '<button type="button" onClick="bos.rptkreditjadwal.cmdpilih(\''.$dbr['rekening'].'\')"
	...
	public function pilih(){
		...
		if(!empty($data)){
		  echo('
				with(bos.rptkreditjadwal.obj){
					find("#rekening").val("'.$data['rekening'].'") ; 
					find("#kode_transaksi").focus() ; 
				}
				bos.rptkreditjadwal.loadmodelstock("hide");
  ...
  public function showreport(){
      $data = getsession($this,"rptkreditjadwal_rpt") ;     
