<?php
class Trakuntansipb extends Bismillah_Controller{
	protected $bdb ;
	public function __construct(){
		parent::__construct() ;
		$this->load->model("tr/trakuntansipb_m") ;
		$this->bdb 	= $this->trakuntansipb_m ;
	}

	public function index(){
		$this->load->view("tr/trakuntansipb") ; 
	...
	public function loadgrid(){
      ...
      while( $dbr = $this->bdb->getrow($dbd) ){
         ...
         $vs['cmddelete']  = '<button type="button" onClick="bos.trakuntansipb.cmddelete(\''.$dbr['id'].'\')"
	...
	public function init(){
		savesession($this, "sstrakuntansipb_id", "") ;    
	} 

	public function saving(){
		$va 	= $this->input->post() ;  
 		$id 	= getsession($this, "sstrakuntansipb_id") ;  
	...
	public function deleting(){
		...
		echo(' bos.trakuntansipb.grid1_reload() ; ') ; 
	}
      	...
      	if($dbra  = $this->bdb->getrow($dba)){ 
			...
			if($total == 0){
				...
				echo(' 
					...
					bos.trakuntansipb.grid1_reload() ; 
					') ;    
			}else{
				echo(' 
